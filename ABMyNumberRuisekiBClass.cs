...
// * 業務名          宛名管理システム
// * 
// * クラス名        ＡＢ共通番号累積マスタＤＡ(ABMyNumberRuisekiBClass)
// * 
// * バージョン情報   Ver 1.0
// * 
...
// ************************************************************************************************
// * 修正履歴     履歴番号    修正内容
// * 2015/07/16   000001      抽出メソッドを公開する（石合）
// ************************************************************************************************
...
/// </summary>
/// <remarks></remarks>
    public class ABMyNumberRuisekiBClass
    {
        ...
        // コンスタント定義
        private const string THIS_CLASS_NAME = "ABMyNumberRuisekiBClass";     // クラス名
    ...
    /// <remarks></remarks>
        public ABMyNumberRuisekiBClass(UFControlData cfControlData, UFConfigDataClass cfConfigDataClass, UFRdbClass cfRdbClass)
    ...
    /// <remarks></remarks>
...

        {
            ...
            string strSQL;
            DataSet csMyNumberRuisekiEntity;
            ...
            try
            {
                ...
                // SQLの実行 DataSetの取得
                csMyNumberRuisekiEntity = m_csDataSchema.Clone();
                csMyNumberRuisekiEntity = m_cfRdbClass.GetDataSet(strSQL, csMyNumberRuisekiEntity, ABMyNumberRuisekiEntity.TABLE_NAME, cfParamCollection, false);
            ...
            // 抽出結果DataSetの返信
            return csMyNumberRuisekiEntity;
    ...
    /// <remarks></remarks>
...

        {
            ...
            try
            {
                ...
                // 住民コード
                csSQL.AppendFormat("{0} = {1} ", ABMyNumberRuisekiEntity.JUMINCD, ABMyNumberRuisekiEntity.PARAM_JUMINCD);

                cfParam = new UFParameterClass();
                cfParam.ParameterName = ABMyNumberRuisekiEntity.PARAM_JUMINCD;
                cfParam.Value = strJuminCd;
                m_cfSelectParamCollection.Add(cfParam);

                // 共通番号
                csSQL.Append("AND ");
                csSQL.AppendFormat("{0} = {1} ", ABMyNumberRuisekiEntity.MYNUMBER, ABMyNumberRuisekiEntity.PARAM_MYNUMBER);

                cfParam = new UFParameterClass();
                cfParam.ParameterName = ABMyNumberRuisekiEntity.PARAM_MYNUMBER;
                cfParam.Value = strMyNumber.RPadRight(13);
                m_cfSelectParamCollection.Add(cfParam);

                // 処理日時
                csSQL.Append("AND ");
                csSQL.AppendFormat("{0} = {1} ", ABMyNumberRuisekiEntity.SHORINICHIJI, ABMyNumberRuisekiEntity.PARAM_SHORINICHIJI);

                cfParam = new UFParameterClass();
                cfParam.ParameterName = ABMyNumberRuisekiEntity.PARAM_SHORINICHIJI;
                cfParam.Value = strShoriNichiji;
                m_cfSelectParamCollection.Add(cfParam);

                // 前後区分
                csSQL.Append("AND ");
                csSQL.AppendFormat("{0} = {1} ", ABMyNumberRuisekiEntity.ZENGOKB, ABMyNumberRuisekiEntity.PARAM_ZENGOKB);

                cfParam = new UFParameterClass();
                cfParam.ParameterName = ABMyNumberRuisekiEntity.PARAM_ZENGOKB;
                cfParam.Value = strZengoKB;

                m_cfSelectParamCollection.Add(cfParam);

                // 抽出処理を実行
                csMyNumberRuisekiEntity = Select(csSQL.ToString(), m_cfSelectParamCollection);
    ...
    /// <remarks></remarks>
        private void CreateSelectSQL()
        {
            ...
            try
            {
                ...
                // FROM区の生成
                csSQL.AppendFormat(" FROM {0}", ABMyNumberRuisekiEntity.TABLE_NAME);

                // スキーマの取得
                if (m_csDataSchema is null)
                {
                    m_csDataSchema = m_cfRdbClass.GetTableSchemaNoRestriction(csSQL.ToString(), ABMyNumberRuisekiEntity.TABLE_NAME, false);
                }
                else
                {
                    // noop
                ...
                csSQL.AppendFormat(" {0},", ABMyNumberRuisekiEntity.JUMINCD);
                csSQL.AppendFormat(" {0},", ABMyNumberRuisekiEntity.MYNUMBER);
                csSQL.AppendFormat(" {0},", ABMyNumberRuisekiEntity.SHORINICHIJI);
                csSQL.AppendFormat(" {0}", ABMyNumberRuisekiEntity.ZENGOKB);
    ...
    /// <remarks></remarks>
        private string CreateSelect()
        {
            ...
            try
            {
                ...
                csSQL.Append("SELECT ");
                csSQL.Append(ABMyNumberRuisekiEntity.JUMINCD);
                csSQL.AppendFormat(", {0}", ABMyNumberRuisekiEntity.SHICHOSONCD);
                csSQL.AppendFormat(", {0}", ABMyNumberRuisekiEntity.KYUSHICHOSONCD);
                csSQL.AppendFormat(", {0}", ABMyNumberRuisekiEntity.MYNUMBER);
                csSQL.AppendFormat(", {0}", ABMyNumberRuisekiEntity.SHORINICHIJI);
                csSQL.AppendFormat(", {0}", ABMyNumberRuisekiEntity.SHORIKB);
                csSQL.AppendFormat(", {0}", ABMyNumberRuisekiEntity.ZENGOKB);
                csSQL.AppendFormat(", {0}", ABMyNumberRuisekiEntity.CKINKB);
                csSQL.AppendFormat(", {0}", ABMyNumberRuisekiEntity.IDOKB);
                csSQL.AppendFormat(", {0}", ABMyNumberRuisekiEntity.IDOYMD);
                csSQL.AppendFormat(", {0}", ABMyNumberRuisekiEntity.IDOSHA);
                csSQL.AppendFormat(", {0}", ABMyNumberRuisekiEntity.RESERVE);
                csSQL.AppendFormat(", {0}", ABMyNumberRuisekiEntity.TANMATSUID);
                csSQL.AppendFormat(", {0}", ABMyNumberRuisekiEntity.SAKUJOFG);
                csSQL.AppendFormat(", {0}", ABMyNumberRuisekiEntity.KOSHINCOUNTER);
                csSQL.AppendFormat(", {0}", ABMyNumberRuisekiEntity.SAKUSEINICHIJI);
                csSQL.AppendFormat(", {0}", ABMyNumberRuisekiEntity.SAKUSEIUSER);
                csSQL.AppendFormat(", {0}", ABMyNumberRuisekiEntity.KOSHINNICHIJI);

                csSQL.AppendFormat(", {0}", ABMyNumberRuisekiEntity.KOSHINUSER);
            }
    ...
    /// <remarks></remarks>
        public int Insert(DataRow csDataRow)
        {
            ...
            try
            {
                ...
                // 共通項目の編集を行う
                csDataRow[ABMyNumberRuisekiEntity.TANMATSUID] = m_cfControlData.m_strClientId;     // 端末ＩＤ
                csDataRow[ABMyNumberRuisekiEntity.SAKUJOFG] = SAKUJOFG_OFF;                        // 削除フラグ
                csDataRow[ABMyNumberRuisekiEntity.KOSHINCOUNTER] = KOSHINCOUNTER_DEF;              // 更新カウンター
                csDataRow[ABMyNumberRuisekiEntity.SAKUSEINICHIJI] = strUpdateDatetime;             // 作成日時
                csDataRow[ABMyNumberRuisekiEntity.SAKUSEIUSER] = m_cfControlData.m_strUserId;      // 作成ユーザー
                csDataRow[ABMyNumberRuisekiEntity.KOSHINNICHIJI] = strUpdateDatetime;              // 更新日時
                csDataRow[ABMyNumberRuisekiEntity.KOSHINUSER] = m_cfControlData.m_strUserId;       // 更新ユーザー

                // パラメータコレクションへ値の設定
                foreach (UFParameterClass cfParam in m_cfInsertParamCollection)
                    cfParam.Value = csDataRow[cfParam.ParameterName.RSubstring(ABMyNumberRuisekiEntity.PARAM_PLACEHOLDER.RLength())].ToString();
        ...
        private void CreateInsertSQL(DataRow csDataRow)
        {
            ...
            try
            {
                ...
                foreach (DataColumn csDataColumn in csDataRow.Table.Columns)
                {
                    ...
                    strParamName = string.Concat(ABMyNumberRuisekiEntity.PARAM_PLACEHOLDER, csDataColumn.ColumnName);
                ...
                m_strInsertSQL = string.Format("INSERT INTO {0} ({1}) VALUES ({2})", ABMyNumberRuisekiEntity.TABLE_NAME, string.Join(',', (string[])csColumnList.ToArray(typeof(string))), string.Join(',', (string[])csParamList.ToArray(typeof(string))));
