...
/// <remarks></remarks>
    public class ABRenrakusakiFZYBClass
    {
        ...
        // コンスタント定義
        private const string THIS_CLASS_NAME = "ABRenrakusakiFZYBClass";              // クラス名
        ...
        private static readonly string SQL_SAKUJOFG = string.Format("{0} = '0'", ABRenrakusakiFZYEntity.SAKUJOFG);
    ...
    /// <remarks></remarks>
        public ABRenrakusakiFZYBClass(UFControlData cfControlData, UFConfigDataClass cfConfigDataClass, UFRdbClass cfRdbClass)
    ...
    /// <returns>抽出結果DataSet</returns>
    /// <remarks></remarks>
        public DataSet SelectByKey(string strJuminCD, string strGyomuCD, string strGyomuNaiShuCD)


        {
            return SelectByKey(strJuminCD, strGyomuCD, strGyomuNaiShuCD, false);
        }
    ...
    /// <returns>抽出結果DataSet</returns>
    /// <remarks></remarks>
        public DataSet SelectByKey(string strJuminCD, string strGyomuCD, string strGyomuNaiShuCD, string strTorokuRenban)



        {
            return SelectByKey(strJuminCD, strGyomuCD, strGyomuNaiShuCD, strTorokuRenban, false);
        }
    ...
    /// <returns>抽出結果DataSet</returns>
    /// <remarks></remarks>
        public DataSet SelectByKey(string strJuminCD, string strGyomuCD, string strGyomuNaiShuCD, bool blnSakujoFG)
...

        {
            ...
            try
            {
                ...
                // 住民コード
                csSQL.AppendFormat("{0} = {1} ", ABRenrakusakiFZYEntity.JUMINCD, ABRenrakusakiFZYEntity.PARAM_JUMINCD);
                ...
                // 業務コード
                csSQL.Append("AND ");
                csSQL.AppendFormat("{0} = {1} ", ABRenrakusakiFZYEntity.GYOMUCD, ABRenrakusakiFZYEntity.PARAM_GYOMUCD);
                ...
                // 業務内種別コード
                csSQL.Append("AND ");
                csSQL.AppendFormat("{0} = {1} ", ABRenrakusakiFZYEntity.GYOMUNAISHU_CD, ABRenrakusakiFZYEntity.PARAM_GYOMUNAISHU_CD);
                ...
                // 抽出処理を実行
                csRenrakusakiFZYEntity = Select(csSQL.ToString(), m_cfSelectParamCollection);
    ...
    /// <remarks></remarks>
...

        {
            ...
            try
            {
                ...
                // 登録連番
                csSQL.Append("AND ");
                csSQL.AppendFormat("{0} = {1} ", ABRenrakusakiFZYEntity.TOROKURENBAN, ABRenrakusakiFZYEntity.PARAM_TOROKURENBAN);
    ...
    /// <remarks></remarks>
        private string CreateSelect()
        {
            ...
            try
            {
                ...
                csSQL.Append("SELECT ");
                csSQL.Append(ABRenrakusakiFZYEntity.JUMINCD);
                csSQL.AppendFormat(", {0}", ABRenrakusakiFZYEntity.SHICHOSONCD);
                csSQL.AppendFormat(", {0}", ABRenrakusakiFZYEntity.KYUSHICHOSONCD);
                csSQL.AppendFormat(", {0}", ABRenrakusakiFZYEntity.GYOMUCD);
                csSQL.AppendFormat(", {0}", ABRenrakusakiFZYEntity.GYOMUNAISHU_CD);
                // *履歴番号 000001 2024/01/11 追加開始
                csSQL.AppendFormat(", {0}", ABRenrakusakiFZYEntity.TOROKURENBAN);
                // *履歴番号 000001 2024/01/11 追加終了
                csSQL.AppendFormat(", {0}", ABRenrakusakiFZYEntity.RENRAKUSAKI4);
                csSQL.AppendFormat(", {0}", ABRenrakusakiFZYEntity.RENRAKUSAKI5);
                csSQL.AppendFormat(", {0}", ABRenrakusakiFZYEntity.RENRAKUSAKI6);
                csSQL.AppendFormat(", {0}", ABRenrakusakiFZYEntity.RENRAKUSAKI1BIKO);
                csSQL.AppendFormat(", {0}", ABRenrakusakiFZYEntity.RENRAKUSAKI2BIKO);
                csSQL.AppendFormat(", {0}", ABRenrakusakiFZYEntity.RENRAKUSAKI3BIKO);
                csSQL.AppendFormat(", {0}", ABRenrakusakiFZYEntity.RENRAKUSAKI4BIKO);
                csSQL.AppendFormat(", {0}", ABRenrakusakiFZYEntity.RENRAKUSAKI5BIKO);
                csSQL.AppendFormat(", {0}", ABRenrakusakiFZYEntity.RENRAKUSAKI6BIKO);
                csSQL.AppendFormat(", {0}", ABRenrakusakiFZYEntity.RESERVE);
                csSQL.AppendFormat(", {0}", ABRenrakusakiFZYEntity.TANMATSUID);
                csSQL.AppendFormat(", {0}", ABRenrakusakiFZYEntity.SAKUJOFG);
                csSQL.AppendFormat(", {0}", ABRenrakusakiFZYEntity.KOSHINCOUNTER);
                csSQL.AppendFormat(", {0}", ABRenrakusakiFZYEntity.SAKUSEINICHIJI);
                csSQL.AppendFormat(", {0}", ABRenrakusakiFZYEntity.SAKUSEIUSER);
                csSQL.AppendFormat(", {0}", ABRenrakusakiFZYEntity.KOSHINNICHIJI);

                csSQL.AppendFormat(", {0}", ABRenrakusakiFZYEntity.KOSHINUSER);
            }
    ...
    /// <remarks></remarks>
        private void CreateInsertSQL(DataRow csDataRow)
        {
            ...
            try
            {
                ...
                m_strInsertSQL = string.Format("INSERT INTO {0} ({1}) VALUES ({2})", ABRenrakusakiFZYEntity.TABLE_NAME, string.Join(',', (string[])csColumnList.ToArray(typeof(string))), string.Join(',', (string[])csParamList.ToArray(typeof(string))));
    ...
    /// <remarks></remarks>
        public int Update(DataRow csDataRow)
        {
            ...
            try
            {
                ...
                // 共通項目の編集を行う
                csDataRow(ABRenrakusakiFZYEntity.TANMATSUID) = m_cfControlData.m_strClientId;                                                            // 端末ＩＤ
                csDataRow(ABRenrakusakiFZYEntity.KOSHINCOUNTER) = Conversions.ToDecimal(csDataRow(ABRenrakusakiFZYEntity.KOSHINCOUNTER)) + 1m;                   // 更新カウンタ
                csDataRow(ABRenrakusakiFZYEntity.KOSHINNICHIJI) = GetValue(csDataRow(ABRenrakusakiFZYEntity.KOSHINNICHIJI), strUpdateDatetime);          // 更新日時
                csDataRow(ABRenrakusakiFZYEntity.KOSHINUSER) = m_cfControlData.m_strUserId;                                                              // 更新ユーザー
                ...
                foreach (UFParameterClass cfParam in m_cfUpdateParamCollection)
                {
                    ...
                    if (cfParam.ParameterName.StartsWith(ABRenrakusakiFZYEntity.PREFIX_KEY, StringComparison.CurrentCulture) == true)
                    {
    ...
    /// <remarks></remarks>
        private void CreateUpdateSQL(DataRow csDataRow)
        {
            ...
            try
            {
                ...
                m_strUpdateSQL = string.Format("UPDATE {0} SET {1} ", ABRenrakusakiFZYEntity.TABLE_NAME, string.Join(',', (string[])csParamList.ToArray(typeof(string))));


                csWhere = new StringBuilder(256);
                csWhere.Append("WHERE ");
                csWhere.AppendFormat("{0} = {1} ", ABRenrakusakiFZYEntity.JUMINCD, ABRenrakusakiFZYEntity.KEY_JUMINCD);
                csWhere.Append("AND ");
                csWhere.AppendFormat("{0} = {1} ", ABRenrakusakiFZYEntity.GYOMUCD, ABRenrakusakiFZYEntity.KEY_GYOMUCD);
                csWhere.Append("AND ");
                csWhere.AppendFormat("{0} = {1} ", ABRenrakusakiFZYEntity.GYOMUNAISHU_CD, ABRenrakusakiFZYEntity.KEY_GYOMUNAISHU_CD);
                // *履歴番号 000001 2024/01/11 追加開始
                csWhere.Append("AND ");
                csWhere.AppendFormat("{0} = {1} ", ABRenrakusakiFZYEntity.TOROKURENBAN, ABRenrakusakiFZYEntity.KEY_TOROKURENBAN);
                // *履歴番号 000001 2024/01/11 追加終了
                csWhere.Append("AND ");
                csWhere.AppendFormat("{0} = {1}", ABRenrakusakiFZYEntity.KOSHINCOUNTER, ABRenrakusakiFZYEntity.KEY_KOSHINCOUNTER);
    ...
    /// <remarks></remarks>
        private void CreateDeleteSQL(DataRow csDataRow)
        {
            ...
            try
            {
                ...
                csSQL = new StringBuilder(256);
                csSQL.AppendFormat("DELETE FROM {0} ", ABRenrakusakiFZYEntity.TABLE_NAME);
                csSQL.Append("WHERE ");
                csSQL.AppendFormat("{0} = {1} ", ABRenrakusakiFZYEntity.JUMINCD, ABRenrakusakiFZYEntity.KEY_JUMINCD);
                csSQL.Append("AND ");
                csSQL.AppendFormat("{0} = {1} ", ABRenrakusakiFZYEntity.GYOMUCD, ABRenrakusakiFZYEntity.KEY_GYOMUCD);
                csSQL.Append("AND ");
                csSQL.AppendFormat("{0} = {1} ", ABRenrakusakiFZYEntity.GYOMUNAISHU_CD, ABRenrakusakiFZYEntity.KEY_GYOMUNAISHU_CD);
                // *履歴番号 000001 2024/01/11 追加開始
                csSQL.Append("AND ");
                csSQL.AppendFormat("{0} = {1} ", ABRenrakusakiFZYEntity.TOROKURENBAN, ABRenrakusakiFZYEntity.KEY_TOROKURENBAN);
                // *履歴番号 000001 2024/01/11 追加終了
                csSQL.Append("AND ");
                csSQL.AppendFormat("{0} = {1}", ABRenrakusakiFZYEntity.KOSHINCOUNTER, ABRenrakusakiFZYEntity.KEY_KOSHINCOUNTER);
    ...
    /// <remarks></remarks>
        private void CreateLogicalDeleteRecoverSQL(DataRow csDataRow)
        {
            ...
            try
            {
                ...
                csSQL = new StringBuilder(256);
                csSQL.AppendFormat("UPDATE {0} ", ABRenrakusakiFZYEntity.TABLE_NAME);
                csSQL.Append("SET ");
                csSQL.AppendFormat("{0} = {1} ", ABRenrakusakiFZYEntity.TANMATSUID, ABRenrakusakiFZYEntity.PARAM_TANMATSUID);
                csSQL.Append(", ");
                csSQL.AppendFormat("{0} = {1} ", ABRenrakusakiFZYEntity.SAKUJOFG, ABRenrakusakiFZYEntity.PARAM_SAKUJOFG);
                csSQL.Append(", ");
                csSQL.AppendFormat("{0} = {1} ", ABRenrakusakiFZYEntity.KOSHINCOUNTER, ABRenrakusakiFZYEntity.PARAM_KOSHINCOUNTER);
                csSQL.Append(", ");
                csSQL.AppendFormat("{0} = {1} ", ABRenrakusakiFZYEntity.KOSHINNICHIJI, ABRenrakusakiFZYEntity.PARAM_KOSHINNICHIJI);
                csSQL.Append(", ");
                csSQL.AppendFormat("{0} = {1} ", ABRenrakusakiFZYEntity.KOSHINUSER, ABRenrakusakiFZYEntity.PARAM_KOSHINUSER);
                csSQL.Append("WHERE ");
                csSQL.AppendFormat("{0} = {1} ", ABRenrakusakiFZYEntity.JUMINCD, ABRenrakusakiFZYEntity.KEY_JUMINCD);
                csSQL.Append("AND ");
                csSQL.AppendFormat("{0} = {1} ", ABRenrakusakiFZYEntity.GYOMUCD, ABRenrakusakiFZYEntity.KEY_GYOMUCD);
