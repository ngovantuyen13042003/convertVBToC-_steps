...
#Region "メンバ変数"
    ...
    'コンスタント定義
    Private Const THIS_CLASS_NAME As String = "ABAtenaJiteBClass"
#End Region
...
#Region "メソッド"
    ...
    Public Function GetAtenaJite(ByVal strJuminCD As String) As DataSet
        Const THIS_METHOD_NAME As String = "GetAtenaJite"
        Dim csAtenaJiteEntity As DataSet
    ...
    Public Function InsertAtenaJite(ByVal csDataRow As DataRow) As Integer
        Const THIS_METHOD_NAME As String = "InsertAtenaJite"
        ...
        Try
            ...
            For Each cfParam In m_cfInsertUFParameterCollectionClass
                m_cfInsertUFParameterCollectionClass(cfParam.ParameterName).Value = csDataRow(cfParam.ParameterName.RSubstring(ABAtenaJiteEntity.PARAM_PLACEHOLDER.RLength)).ToString()
            Next cfParam
    ...
    Public Function UpdateAtenaJite(ByVal csDataRow As DataRow) As Integer
        ...
        Try
            ...
            ' 共通項目の編集を行う
            csDataRow(ABAtenaJiteEntity.TANMATSUID) = m_cfControlData.m_strClientId                                 '端末ＩＤ
            csDataRow(ABAtenaJiteEntity.KOSHINCOUNTER) = CDec(csDataRow(ABAtenaJiteEntity.KOSHINCOUNTER)) + 1       '更新カウンタ
            ...
            For Each cfParam In m_cfUpdateUFParameterCollectionClass
                ' キー項目は更新前の値で設定
                If (cfParam.ParameterName.RSubstring(0, ABAtenaJiteEntity.PREFIX_KEY.RLength) = ABAtenaJiteEntity.PREFIX_KEY) Then
                    m_cfUpdateUFParameterCollectionClass(cfParam.ParameterName).Value =
                            csDataRow(cfParam.ParameterName.RSubstring(ABAtenaJiteEntity.PREFIX_KEY.RLength),
                ...
                Else
                    ...
                    '*履歴番号 000005 2004/11/11 修正終了
                    ' パラメータコレクションへ値の設定
                    m_cfUpdateUFParameterCollectionClass(cfParam.ParameterName).Value = csDataRow(cfParam.ParameterName.RSubstring(ABAtenaJiteEntity.PARAM_PLACEHOLDER.RLength), DataRowVersion.Current).ToString()
                End If
            Next cfParam
    ...
    Private Sub CheckColumnValue(ByVal strColumnName As String, ByVal strValue As String)
        ...
        Try
            ...
            Select Case strColumnName.ToUpper()
                Case ABAtenaJiteEntity.JUMINCD                          '住民コード
                    ...
                    End If
                Case ABAtenaJiteEntity.SHICHOSONCD                      '市町村コード
                    If (Not UFStringClass.CheckNumber(strValue)) Then
                        m_cfErrorClass = New UFErrorClass(ABConstClass.THIS_BUSINESSID)
                        'エラー定義を取得
                        ...
                        Throw New UFAppException(objErrorStruct.m_strErrorMessage, objErrorStruct.m_strErrorCode)
                    End If
                Case ABAtenaJiteEntity.KYUSHICHOSONCD                   '旧市町村コード
                    If (Not UFStringClass.CheckNumber(strValue)) Then
                        m_cfErrorClass = New UFErrorClass(ABConstClass.THIS_BUSINESSID)
                        'エラー定義を取得
                ...
                Case ABAtenaJiteEntity.JIDOTEATEHIYOKB                  '児手被用区分
                    If (Not UFStringClass.CheckNumber(strValue)) Then
                        m_cfErrorClass = New UFErrorClass(ABConstClass.THIS_BUSINESSID)
                        'エラー定義を取得
                ...
                Case ABAtenaJiteEntity.JIDOTEATESTYM                    '児手開始年月
                    If Not (strValue = String.Empty Or strValue = "000000") Then
                        ...
                        End If
                    End If
                Case ABAtenaJiteEntity.JIDOTEATEEDYM                    '児手終了年月
                    If Not (strValue = String.Empty Or strValue = "000000") Then
                        m_cfDateClass.p_strDateValue = strValue + "01"
                        If (Not m_cfDateClass.CheckDate()) Then
                            m_cfErrorClass = New UFErrorClass(ABConstClass.THIS_BUSINESSID)
                        ...
                        End If
                    End If
                Case ABAtenaJiteEntity.TANMATSUID                       '端末ID
                    If (Not UFStringClass.CheckANK(strValue)) Then
                        ...
                        Throw New UFAppException(objErrorStruct.m_strErrorMessage, objErrorStruct.m_strErrorCode)
                    End If
                Case ABAtenaJiteEntity.SAKUJOFG                         '削除フラグ
                    If (Not UFStringClass.CheckNumber(strValue)) Then
                        m_cfErrorClass = New UFErrorClass(ABConstClass.THIS_BUSINESSID)
                        'エラー定義を取得
                        ...
                        Throw New UFAppException(objErrorStruct.m_strErrorMessage, objErrorStruct.m_strErrorCode)
                    End If
                Case ABAtenaJiteEntity.KOSHINCOUNTER                    '更新カウンタ
                    If (Not UFStringClass.CheckNumber(strValue)) Then
                        m_cfErrorClass = New UFErrorClass(ABConstClass.THIS_BUSINESSID)
                        'エラー定義を取得
                        ...
                        Throw New UFAppException(objErrorStruct.m_strErrorMessage, objErrorStruct.m_strErrorCode)
                    End If
                Case ABAtenaJiteEntity.SAKUSEINICHIJI                   '作成日時
                    If (Not UFStringClass.CheckNumber(strValue)) Then
                        m_cfErrorClass = New UFErrorClass(ABConstClass.THIS_BUSINESSID)
                        'エラー定義を取得
                        ...
                        Throw New UFAppException(objErrorStruct.m_strErrorMessage, objErrorStruct.m_strErrorCode)
                    End If
                Case ABAtenaJiteEntity.SAKUSEIUSER                      '作成ユーザ
                    If (Not UFStringClass.CheckANK(strValue)) Then
                        m_cfErrorClass = New UFErrorClass(ABConstClass.THIS_BUSINESSID)
                        'エラー定義を取得
                        ...
                        Throw New UFAppException(objErrorStruct.m_strErrorMessage, objErrorStruct.m_strErrorCode)
                    End If
                Case ABAtenaJiteEntity.KOSHINNICHIJI                    '更新日時
                    If (Not UFStringClass.CheckNumber(strValue)) Then
                        m_cfErrorClass = New UFErrorClass(ABConstClass.THIS_BUSINESSID)
                        'エラー定義を取得
                        ...
                        Throw New UFAppException(objErrorStruct.m_strErrorMessage, objErrorStruct.m_strErrorCode)
                    End If
                Case ABAtenaJiteEntity.KOSHINUSER                       '更新ユーザ
                    If (Not UFStringClass.CheckANK(strValue)) Then
                        m_cfErrorClass = New UFErrorClass(ABConstClass.THIS_BUSINESSID)
                        'エラー定義を取得
